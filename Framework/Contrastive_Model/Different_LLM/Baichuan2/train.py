...
from model import Baichuan_Roberta_Encoder
...
def get_argument():
    ...
    parser.add_argument('-train_dataset_path', type=str,
                        default='/mnt/data132/taozhen/AI_Thesis_Detection/Dataset/HPPT/Baichuan2/train.json')
    parser.add_argument('-val_dataset_path', type=str,
                        default='/mnt/data132/taozhen/AI_Thesis_Detection/Dataset/HPPT/Baichuan2/val.json')
...
def twice_triple_loss(anchor, positive1, positive2, negative, margin=1.0):
    ...
    # 计算 anchor 和负样本的余弦距离
    neg_dist1 = 1 - F.cosine_similarity(anchor, negative)

    # 计算正样本与负样本之间的余弦距离，增强负样本效用
    neg_dist2 = 1 - F.cosine_similarity(positive1, negative)
    neg_dist3 = 1 - F.cosine_similarity(positive2, negative)

    # 计算损失，使用 margin 和 clamp 避免负损失
    loss1 = torch.clamp(margin + pos_dist1 - neg_dist1, min=0.0)
    loss2 = torch.clamp(margin + pos_dist2 - neg_dist1, min=0.0)

    # 添加正样本和负样本之间的距离损失
    loss1 += torch.clamp(margin + pos_dist1 - neg_dist2, min=0.0)
    loss2 += torch.clamp(margin + pos_dist2 - neg_dist3, min=0.0)
...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'baichuan_roberta_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'baichuan_roberta_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'baichuan_roberta_save', 'run_' + str(run_id))
    ...
    ########################  Model ###########################
    model = Baichuan_Roberta_Encoder(args.encoder_model_path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        for step, sample_batched in enumerate(train_loader):
            ...
            Human_text_encoder = model(input_ids=input_ids_1, attention_mask=attention_mask_1)
            AI_text_encoder = model(input_ids=input_ids_2, attention_mask=attention_mask_2)
            AI_rewrite_encoder = model(input_ids=input_ids_3, attention_mask=attention_mask_3)
            AI_humanlike_encoder = model(input_ids=input_ids_4, attention_mask=attention_mask_4)
        ...
        for step, sample_batched in enumerate(val_loader):
            ...
            if step % num_iter_ts == num_iter_ts - 1:
                ...
                cur_f = mean_valLoss
                if cur_f < best_f:
                    save_path = os.path.join(save_dir, 'contra_baichuan_roberta_model' + '.pth')
