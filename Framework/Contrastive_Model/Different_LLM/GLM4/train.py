...
from model import GLM_Roberta_Encoder
...
def get_argument():
    ...
    parser.add_argument('-train_dataset_path', type=str,
                        default='/mnt/data132/taozhen/AI_Thesis_Detection/Dataset/HPPT/GLM4/train.json')
    parser.add_argument('-val_dataset_path', type=str,
                        default='/mnt/data132/taozhen/AI_Thesis_Detection/Dataset/HPPT/GLM4/val.json')
...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'glm_roberta_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'glm_roberta_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'glm_roberta_save', 'run_' + str(run_id))
    ...
    ########################  Model ###########################
    model = GLM_Roberta_Encoder(args.encoder_model_path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        for step, sample_batched in enumerate(val_loader):
            ...
            if step % num_iter_ts == num_iter_ts - 1:
                ...
                cur_f = mean_valLoss
                if cur_f < best_f:
                    save_path = os.path.join(save_dir, 'contra_glm_roberta_model' + '.pth')
