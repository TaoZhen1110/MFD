...
from model import Llama_Roberta_Encoder
...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'llama_roberta_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'llama_roberta_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'llama_roberta_save', 'run_' + str(run_id))
    ...
    ########################  Model ###########################
    model = Llama_Roberta_Encoder(args.encoder_model_path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        for step, sample_batched in enumerate(val_loader):
            ...
            if step % num_iter_ts == num_iter_ts - 1:
                ...
                cur_f = mean_valLoss
                if cur_f < best_f:
                    save_path = os.path.join(save_dir, 'contra_llama_roberta_model' + '.pth')
