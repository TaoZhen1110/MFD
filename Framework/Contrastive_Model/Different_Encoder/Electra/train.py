...
from model import Electra_Encoder
...
def main(args):
    ...
    ###################  Dataset prepare #################
    tokenizer = ElectraTokenizer.from_pretrained(args.encoder_model_path)
    ...
    ########################  Model ###########################
    model = Electra_Encoder(args.encoder_model_path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        for step, sample_batched in enumerate(val_loader):
            ...
            if step % num_iter_ts == num_iter_ts - 1:
                ...
                cur_f = mean_valLoss
                if cur_f < best_f:
                    save_path = os.path.join(save_dir, 'contra_electra_model' + '.pth')
