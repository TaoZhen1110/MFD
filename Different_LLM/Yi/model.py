...
################  加载第一个分支模型  ##################
class Yi_T5_Encoder(nn.Module):
    def __init__(self, T5_Path):
        super(Yi_T5_Encoder, self).__init__()
        # 加载预训练的 T5 编码器模型
...
class Multi_Level_Framework(nn.Module):
    def __init__(self, T5_Path, Yi_T5_Encoder_Path):
        super(Multi_Level_Framework, self).__init__()

        ##########  第一个分支
        self.First_Level = Yi_T5_Encoder(T5_Path=T5_Path)

        state_dict = torch.load(Yi_T5_Encoder_Path)
