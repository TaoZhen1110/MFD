...
def get_argument():
    ...
    parser.add_argument('-GLM_T5_Encoder_Path', type=str,
    ...
    parser.add_argument('-train_dataset_path', type=str,
                        default="/mnt/data132/taozhen/AI_Thesis_Detection/Dataset/PASTED/GLM4/train.json")
    parser.add_argument('-val_dataset_path', type=str,
                        default="/mnt/data132/taozhen/AI_Thesis_Detection/Dataset/PASTED/GLM4/val.json")
...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'GLM_t5_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'GLM_t5_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'GLM_t5_save', 'run_' + str(run_id))
    ...
    ########################  Model ###########################
    model = Multi_Level_Framework(args.T5_Path, args.GLM_T5_Encoder_Path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        if Mean_mse < best_f:  # 如果当前MSE小于之前的最佳值
            save_path = os.path.join(save_dir, 'glm_t5' + '.pth')
