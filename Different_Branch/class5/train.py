...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'class5_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'class5_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'class5_save', 'run_' + str(run_id))
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        for step, sample_batched in enumerate(train_loader):
            ...
            output = model(input_ids_1, attention_mask_1, Third_level_feature)
        ...
        if Mean_mse < best_f:  # 如果当前MSE小于之前的最佳值
            save_path = os.path.join(save_dir, 'class5_model' + '.pth')
