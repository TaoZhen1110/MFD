...
class LLM_Semantics_Analysis(nn.Module):
    def __init__(self, DistilBERT_Path):
        ...
        ########### 利用SCIBert对论文编码
        # 加载 DistilBERT 预训练模型，用于 Thesis 编码
        self.DistilBERT_thesis = DistilBertModel.from_pretrained(DistilBERT_Path)
        for name, param in self.DistilBERT_thesis.named_parameters():
            if name.startswith("transformer.layer.5"):  # 修改为 layer.5 来访问最后一层（第 6 层）
                param.requires_grad = True  # 确保只有最后一层的参数可以在训练过程中更新
            else:
                param.requires_grad = False  # 冻结其他层的参数

        # 加载 DistilBERT 预训练模型，用于 LLM 分析
        self.DistilBERT_LLM_analysis = DistilBertModel.from_pretrained(DistilBERT_Path)
        for name, param in self.DistilBERT_LLM_analysis.named_parameters():
            if name.startswith("transformer.layer.5"):  # 同样修改为 layer.5
                param.requires_grad = True  # 确保只有最后一层的参数可以在训练过程中更新
            else:
                param.requires_grad = False  # 冻结其他层的参数
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2):
        ...
        thesis_feature = self.DistilBERT_thesis(input_ids_1, attention_mask_1)[0][:, 0, :]

        LLM_analysis_feature = self.DistilBERT_LLM_analysis(input_ids_2, attention_mask_2)[0][:, 0, :]
...
class Multi_Level_Framework(nn.Module):
    def __init__(self, DistilBERT_Path, Thesis_DistilBERT_Encoder_Path):
        super(Multi_Level_Framework, self).__init__()

        ##########  第一个分支
        self.First_Level = DistilBERT_Encoder(DistilBERT_Path=DistilBERT_Path)

        state_dict = torch.load(Thesis_DistilBERT_Encoder_Path)
        ...
        for i in range(-3, 0):
            # 解冻每一层的参数
            for name, param in self.First_Level.model.transformer.layer[i].named_parameters():
        ...
        ##########  第二个分支
        self.Second_Level = LLM_Semantics_Analysis(DistilBERT_Path)
    ...
    def forward(self, input_ids_1, attention_mask_1,
                input_ids_2, attention_mask_2, third_level_feature):
