...
def get_argument():
    ...
    parser.add_argument('-DistilBERT_Path', type=str,
                        default="/mnt/data132/taozhen/AI_Thesis_Detection/Framework/Contrastive_Model/Different_Encoder/DistilBERT/distilbert_pretrained/")
    parser.add_argument('-Thesis_DistilBERT_Encoder_Path', type=str,
                        default="/mnt/data132/taozhen/AI_Thesis_Detection/Framework/Contrastive_Model/Different_Encoder/DistilBERT/distilbert_save/run_0/contra_distilbert_model.pth")
...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'DistilBERT_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'DistilBERT_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'DistilBERT_save', 'run_' + str(run_id))
    ...
    ###################  Dataset prepare #################
    tokenizer = DistilBertTokenizer.from_pretrained(args.DistilBERT_Path)
    ...
    ########################  Model ###########################
    model = Multi_Level_Framework(args.DistilBERT_Path, args.Thesis_DistilBERT_Encoder_Path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        if Mean_mse < best_f:  # 如果当前MSE小于之前的最佳值
            save_path = os.path.join(save_dir, 'distilbert_model' + '.pth')
