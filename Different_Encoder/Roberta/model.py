...
class LLM_Semantics_Analysis(nn.Module):
    def __init__(self, Roberta_Path):
        ...
        ########### 利用SCIBert对论文编码
        self.Roberta_thesis = RobertaModel.from_pretrained(Roberta_Path)
        for name, param in self.Roberta_thesis.named_parameters():
        ...
        ########### 利用SCIBert对大模型分析进行编码
        self.Roberta_LLM_analysis = RobertaModel.from_pretrained(Roberta_Path)
        for name, param in self.Roberta_LLM_analysis.named_parameters():
            if name.startswith("encoder.layer.11"):
                param.requires_grad = True  # 确保只有第 11 层的参数可以在训练过程中更新。
            else:
                param.requires_grad = False
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2):
        ...
        thesis_feature = self.Roberta_thesis(input_ids_1, attention_mask_1)[0][:, 0, :]

        LLM_analysis_feature = self.Roberta_LLM_analysis(input_ids_2, attention_mask_2)[0][:, 0, :]
...
class Multi_Level_Framework(nn.Module):
    def __init__(self, Roberta_Path, Thesis_Roberta_Encoder_Path):
        ...
        ##########  第一个分支
        self.First_Level = Thesis_Roberta_Encoder(Roberta_Path=Roberta_Path)

        state_dict = torch.load(Thesis_Roberta_Encoder_Path)
        ...
        ##########  第二个分支
        self.Second_Level = LLM_Semantics_Analysis(Roberta_Path)
