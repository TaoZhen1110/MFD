...
class LLM_Semantics_Analysis(nn.Module):
    def __init__(self, ALBERT_Path):
        ...
        ########### 利用SCIBert对论文编码
        self.ALBERT_thesis = AlbertModel.from_pretrained(ALBERT_Path)
        # 冻结所有参数，只有第 11 层的参数不被冻结
        for name, param in self.ALBERT_thesis.named_parameters():
            if name.startswith("encoder.layer.11"):  # ALBERT 的第 11 层
                param.requires_grad = True  # 解冻第 11 层
            else:
                param.requires_grad = False  # 冻结其他层

        ########### 利用SCIBert对大模型分析进行编码
        self.ALBERT_LLM_analysis = AlbertModel.from_pretrained(ALBERT_Path)
        for name, param in self.ALBERT_LLM_analysis.named_parameters():
    ...
    def forward(self, input_ids_1, attention_mask_1, token_type_ids_1,
        ...
        thesis_feature = self.ALBERT_thesis(input_ids_1, attention_mask_1, token_type_ids_1)[0][:, 0, :]

        LLM_analysis_feature = self.ALBERT_LLM_analysis(input_ids_2, attention_mask_2, token_type_ids_2)[0][:, 0, :]
...
class Multi_Level_Framework(nn.Module):
    def __init__(self, ALBERT_Path, Thesis_ALBERT_Encoder_Path):
        super(Multi_Level_Framework, self).__init__()

        ##########  第一个分支
        self.First_Level = ALBERT_Encoder(ALBERT_Path=ALBERT_Path)

        state_dict = torch.load(Thesis_ALBERT_Encoder_Path)
        state_dict = remove_module_prefix(state_dict)
        self.First_Level.load_state_dict(state_dict)

        # 解冻最后 5 层
        # 假设每个 layer group 中只有一个 albert_layer
        total_layers = sum(len(group.albert_layers) for group in self.First_Level.model.encoder.albert_layer_groups)

        # 计算需要解冻的起始位置
        start_layer = total_layers - 5

        current_layer = 0
        for group in self.First_Level.model.encoder.albert_layer_groups:
        ...
        ##########  第二个分支
        self.Second_Level = LLM_Semantics_Analysis(ALBERT_Path)
