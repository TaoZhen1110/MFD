...
class XLNet_Encoder(nn.Module):
    def __init__(self, XLNet_Path):
        super(XLNet_Encoder, self).__init__()
        # 加载预训练的 XLNet 模型
        self.model = XLNetModel.from_pretrained(XLNet_Path)

    def forward(self, input_ids, attention_mask, token_type_ids):
        # 获取 XLNet 模型的输出 (batch_size, seq_length, hidden_size)
...
class LLM_Semantics_Analysis(nn.Module):
    def __init__(self, XLNet_Path):
        ...
        ########### 利用SCIBert对论文编码
        self.XLNet_thesis = XLNetModel.from_pretrained(XLNet_Path)
        # 冻结所有参数，只有第 11 层的参数不被冻结
        for name, param in self.XLNet_thesis.named_parameters():
            if name.startswith("layer.11"):  # XLNet 的第 11 层
                param.requires_grad = True  # 解冻第 11 层
            else:
                param.requires_grad = False  # 冻结其他层
        #
        # ########### 利用SCIBert对大模型分析进行编码
        self.XLNet_LLM_analysis = XLNetModel.from_pretrained(XLNet_Path)
        for name, param in self.XLNet_LLM_analysis.named_parameters():
            if name.startswith("layer.11"):
    ...
    def forward(self, input_ids_1, attention_mask_1, token_type_ids_1,
        ...
        thesis_feature = self.XLNet_thesis(input_ids=input_ids_1, attention_mask=attention_mask_1, token_type_ids=token_type_ids_1)[0][:, 0, :]


        LLM_analysis_feature = self.XLNet_LLM_analysis(input_ids=input_ids_2, attention_mask=attention_mask_2, token_type_ids=token_type_ids_2)[0][:, 0, :]
...
class Multi_Level_Framework(nn.Module):
    def __init__(self, XLNet_Path, Thesis_XLNet_Encoder_Path):
        super(Multi_Level_Framework, self).__init__()

        ##########  第一个分支
        self.First_Level = XLNet_Encoder(XLNet_Path=XLNet_Path)

        state_dict = torch.load(Thesis_XLNet_Encoder_Path)
        ...
        for i in range(-5, 0):  # 解冻最后 5 层
            for param in self.First_Level.model.layer[i].parameters(): # 正确访问 transformer 层
        ...
        ##########  第二个分支
        self.Second_Level = LLM_Semantics_Analysis(XLNet_Path)
