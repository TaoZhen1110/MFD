...
def get_argument():
    ...
    parser.add_argument('-XLNet_Path', type=str,
                        default="/mnt/data132/taozhen/AI_Thesis_Detection/Framework/Contrastive_Model/Different_Encoder/XLNet/xlnet_pretrained/")
    parser.add_argument('-Thesis_XLNet_Encoder_Path', type=str,
                        default="/mnt/data132/taozhen/AI_Thesis_Detection/Framework/Contrastive_Model/Different_Encoder/XLNet/xlnet_save/run_0/contra_xlnet_model.pth")
...
def main(args):
    ...
    if args.resume_epoch != 0:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'XLNet_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) if runs else 0
    else:
        runs = sorted(glob.glob(os.path.join(save_dir_root, 'XLNet_save', 'run_*')))
        run_id = int(runs[-1].split('_')[-1]) + 1 if runs else 0

    save_dir = os.path.join(save_dir_root, 'XLNet_save', 'run_' + str(run_id))
    ...
    ###################  Dataset prepare #################
    tokenizer = XLNetTokenizer.from_pretrained(args.XLNet_Path)
    ...
    ########################  Model ###########################
    model = Multi_Level_Framework(args.XLNet_Path, args.Thesis_XLNet_Encoder_Path)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.epochs)):
        ...
        for step, sample_batched in enumerate(train_loader):
            ...
            output, second_prediction_result = model(input_ids_1, attention_mask_1, token_type_ids_1,
                                                     input_ids_2, attention_mask_2, token_type_ids_2,
                                                     Third_level_feature)

            loss1 = loss_function(output, Label)
            loss2 = loss_function(second_prediction_result, Label)
            final_loss = loss1 + 0.5 * loss2
            count1 += Label.size(0)
        ...
        if Mean_mse < best_f:  # 如果当前MSE小于之前的最佳值
            save_path = os.path.join(save_dir, 'xlnet_model' + '.pth')
